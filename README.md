...
폴더 안 7개 파일이 전부입니다. 정적 호스팅에 그대로 올리면 일반 웹사이트 주소로 열립니다.
...
- 업데이트할 때는 `index.html`, `sw.js`, `version.json` **세 파일을 함께** 올립니다. 앱을 열면 `version.json` 을 보고 `Update available · Tap to reload` 알림을 띄웁니다.
